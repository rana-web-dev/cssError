...
const QuizContent = () => {
    ...
    return (
        <div className='max-w-6xl mx-auto'>
            <ToastContainer />
            <h2 className='text-center text-4xl font-bold text-blue-500 py-10 '>Quiz No: {id} {name}</h2>
            {
    ...
    return (
        <div className='md:w-1/2 w-1/1 mx-auto mb-10'>
            <div className='m-2 md:m-0'>
                <p className='font-bold text-2xl'><span className='bg-red-500 text-white rounded pb-1 px-4 m-2'>Question:</span> {question}</p>
                <div className='text-right'>
                    <button onClick={() => eyeCorrect()}><Eye /></button>
                </div>
                <div className='grid grid-cols-2 cursor-default'>
                    {
                        options.map((op, ind) => (<Options
                            key={ind}
                            option={op}
...
const Options = ({ option, onOptionClick }) => {
    ...
    return (
        <div>
            <p
                onClick={handleClick}
                className='bg-blue-500 m-2 text-white p-2 rounded select-none'>{option}</p>
        </div>
    )
}
